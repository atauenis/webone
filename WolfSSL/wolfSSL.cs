...
namespace wolfSSL.CSharp
{
    ...
    {
        /* Bare name (no extension): .NET's cross-platform native library resolution
         * probes "libwolfssl.so" on Linux, "wolfssl.dylib" on macOS, "wolfssl.dll" on
         * Windows automatically -- an explicit ".dll" suffix would only work on Windows. */
        ...
        [DllImport(wolfssl_dll, CallingConvention = CallingConvention.Cdecl)]
        private extern static void wolfSSL_CTX_free(IntPtr ctx);
        /* Buffer variants -- not in the upstream wrapper, added for WebOne's use case
         * of certificates generated in-memory (never written to disk) via .NET's
         * X509Certificate2 API. */
        [DllImport(wolfssl_dll, CallingConvention = CallingConvention.Cdecl)]
        private extern static int wolfSSL_CTX_use_certificate_buffer(IntPtr ctx, byte[] buf, long sz, int type);
        [DllImport(wolfssl_dll, CallingConvention = CallingConvention.Cdecl)]
        private extern static int wolfSSL_CTX_use_PrivateKey_buffer(IntPtr ctx, byte[] buf, long sz, int type);
        /* wolfSSLv23_server_method()'s "flexible" version range has a default minimum
         * floor above SSLv3 -- SetMinVersion(ctx, WOLFSSL_SSLV3) lowers it so a
         * genuinely SSLv3-only ClientHello (e.g. from a legacy browser configured to
         * offer only SSL 3.0) is actually accepted rather than rejected with a
         * record-layer version error, matching what --enable-sslv3 built the library
         * to support in the first place. */
        [DllImport(wolfssl_dll, CallingConvention = CallingConvention.Cdecl)]
        private extern static int wolfSSL_CTX_SetMinVersion(IntPtr ctx, int version);
        public static readonly int WOLFSSL_SSLV3 = 0;
#endif
        ...
        /// <returns>1 on success</returns>
        /* Widened from the original `Socket fd` parameter to `object fd` -- the body
         * below only ever GCHandle-pins whatever is passed and hands the pinned
         * pointer to wolfSSL as an opaque IO context, so it works identically for a
         * Socket, a Stream, or any other managed object a custom IO callback pair
         * (SetIORecv/SetIOSend) knows how to interpret via that same GCHandle. */
        public static int set_fd(IntPtr ssl, object fd)
        {
        ...
        /// <summary>
        /// Load a certificate from an in-memory DER or PEM buffer (no file on disk).
        /// </summary>
        /// <param name="ctx">CTX structure for TLS/SSL connections</param>
        /// <param name="cert">DER or PEM encoded certificate bytes</param>
        /// <param name="type">SSL_FILETYPE_ASN1 (DER) or SSL_FILETYPE_PEM</param>
        /// <returns>1 on success</returns>
        /// <summary>
        /// Lower the minimum accepted protocol version on a CTX (e.g. to
        /// WOLFSSL_SSLV3, so a "flexible" v23 method actually accepts SSLv3 instead
        /// of only the higher floor it defaults to).
        /// </summary>
        public static int CTX_SetMinVersion(IntPtr ctx, int version)
        {
            try
            {
                ...
                if (local_ctx == IntPtr.Zero)
                {
                    log(ERROR_LOG, "CTX SetMinVersion error");
                    return FAILURE;
                }

                return wolfSSL_CTX_SetMinVersion(local_ctx, version);
            }
            catch (Exception e)
            {
                log(ERROR_LOG, "wolfssl ctx SetMinVersion error " + e.ToString());
                return FAILURE;
            }
        }

        public static int CTX_use_certificate_buffer(IntPtr ctx, byte[] cert, int type)
        {
            try
            {
                IntPtr local_ctx = unwrap_ctx(ctx);
                if (local_ctx == IntPtr.Zero)
                {
                    log(ERROR_LOG, "CTX use certificate buffer error");
                    return FAILURE;
                }

                return wolfSSL_CTX_use_certificate_buffer(local_ctx, cert, cert.Length, type);
            }
            catch (Exception e)
            {
                log(ERROR_LOG, "wolfssl ctx use cert buffer error " + e.ToString());
                return FAILURE;
            }
        }

        /// <summary>
        /// Load a private key from an in-memory DER or PEM buffer (no file on disk).
        /// </summary>
        /// <param name="ctx">CTX structure for TLS/SSL connections</param>
        /// <param name="key">DER or PEM encoded private key bytes</param>
        /// <param name="type">SSL_FILETYPE_ASN1 (DER) or SSL_FILETYPE_PEM</param>
        /// <returns>1 on success</returns>
        public static int CTX_use_PrivateKey_buffer(IntPtr ctx, byte[] key, int type)
        {
            try
            {
                IntPtr local_ctx = unwrap_ctx(ctx);
                if (local_ctx == IntPtr.Zero)
                {
                    log(ERROR_LOG, "CTX use PrivateKey buffer error");
                    return FAILURE;
                }

                return wolfSSL_CTX_use_PrivateKey_buffer(local_ctx, key, key.Length, type);
            }
            catch (Exception e)
            {
                log(ERROR_LOG, "wolfssl ctx use PrivateKey buffer error " + e.ToString());
                return FAILURE;
            }
        }
